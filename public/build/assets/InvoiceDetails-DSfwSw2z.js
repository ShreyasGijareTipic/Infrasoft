...
${(()=>{var W,y,q,ee,f,P,n,l,j,v,b,g,s,r,i,u,w,L,U,C,G,F,I,N,B,Q,H,X,p,oe,re,ie,ae,le,de;const _=Array.isArray(o.items)?o.items:[],O=Math.ceil(_.length/19)||1,D=!!(o.note||o.payment_terms||o.terms_and_conditions);let z="";for(let T=0;T<O;T++){const te=T*19,he=te+19,ge=_.slice(te,he),xe=T===O-1;z+=`
      ...
      <div class="invoice-box">
          ...
          <tr>
            <td style="width: 70%; vertical-align: top;">
              <div style="font-size: 22px; font-weight: bold; line-height: 1.2;">
                ${((y=(W=c())==null?void 0:W.company_info)==null?void 0:y.company_name)||"Company Name"}
              </div>
              <div style="font-size: 11px; margin-top: 2px;">
                ${((ee=(q=c())==null?void 0:q.company_info)==null?void 0:ee.land_mark)||"-"}
              </div>
              <div style="font-size: 11px; margin-top: 3px;">
                <b>Phone:</b> ${((P=(f=c())==null?void 0:f.company_info)==null?void 0:P.phone_no)||"-"}
              </div>
            </td>
        ...
        <table style="border: 1px solid #000; margin: 6px 0;">
          ...
          <tr>
            <td style="border: 1px solid #000; font-size: 10px; padding: 5px; line-height: 1.4;">
              ...
              <b>Phone:</b> ${((r=c())==null?void 0:r.mobile)||"N/A"}<br/>
              <b>GSTIN:</b> ${((u=(i=c())==null?void 0:i.company_info)==null?void 0:u.gst_number)||"N/A"}<br/>
              Dist: ${((L=(w=c())==null?void 0:w.company_info)==null?void 0:L.Dist)||"-"}<br/>
              Tal: ${((C=(U=c())==null?void 0:U.company_info)==null?void 0:C.Tal)||"-"}<br/>
              Email: ${((F=(G=c())==null?void 0:G.company_info)==null?void 0:F.email_id)||"-"}
            </td>
            <td style="border: 1px solid #000; font-size: 10px; padding: 5px; line-height: 1.4;">
              <b>Customer Name:</b> ${((I=o.customer)==null?void 0:I.name)||"Customer Name"}<br/>
              <b>Site:</b> ${o.project_name||"Project Name"}<br/>
              ${((N=o.customer)==null?void 0:N.address)||"Customer Address"}<br/>
              <b>Phone:</b> ${((B=o.customer)==null?void 0:B.mobile)||"N/A"}<br/>
        ...
        <table class="details-table" style="margin-top: 8px;">
          ...
          <thead>
            <tr>
              ...
              ${m?`
                ...
                <th style="width: 12%; font-size: 10px;">${a.cgst}${(Q=_[0])!=null&&Q.gst_percent?` (${_[0].gst_percent/2}%)`:""}</th>
                <th style="width: 12%; font-size: 10px;">${a.sgst}${(H=_[0])!=null&&H.gst_percent?` (${_[0].gst_percent/2}%)`:""}</th>
              `:""}
          ...
          <tbody>
            ${ge.map((h,se)=>`
              <tr>
                <td class="center" style="width: ${m?"5%":"6%"}; font-size: 10px;">${se+1+te}</td>
    ...
    ${t?`
      <table class="summary" style="margin-top: 8px;">
        ...
        <tr>
          ...
          <td class="right" style="font-size: 11px;">₹${(()=>{const h=_.reduce((se,be)=>se+(be.total_price||0),0);return Number(h).toFixed(2)})()}</td>
        </tr>
        ${Number(o.cgst||0)>0?`
        <tr>
          <th style="font-size: 11px;">${a.cgst} (${o.cgstPercentage||Number(o.gst||0)/2}%)</th>
          <td class="right" style="font-size: 11px;">₹${Number(o.cgst||0).toFixed(2)}</td>
        </tr>`:""}
        ${Number(o.sgst||0)>0?`
        <tr>
          <th style="font-size: 11px;">${a.sgst} (${o.sgstPercentage||Number(o.gst||0)/2}%)</th>
      ...
      <tr style="background: #f8d7da;">
        <th style="vertical-align: top; font-size: 11px;">
          ...
          <span style="font-weight: normal; font-size: 10px;">
            ${a.amountInWords} ${Z} ${a.only}
          </span>
        </th>
        <td class="right" style="vertical-align: top; font-size: 11px;">
          <strong>₹${Number(A||0).toFixed(2)}</strong>
        </td>
      </tr>
    </table>
  </div>
      ...
      <div class="content-wrapper">
        <table class="company-header" style="width: 100%; margin-bottom: 5px;">
          <tr>
            <td style="width: 70%; vertical-align: top;">
              <div style="font-size: 22px; font-weight: bold; line-height: 1.2;">
                ${((p=(X=c())==null?void 0:X.company_info)==null?void 0:p.company_name)||"Company Name"}
              </div>
              <div style="font-size: 11px; margin-top: 2px;">
                ${((re=(oe=c())==null?void 0:oe.company_info)==null?void 0:re.land_mark)||"-"}
              </div>
              <div style="font-size: 11px; margin-top: 3px;">
...
`,Y={margin:.5,filename:`${x}_${S}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0},jsPDF:{unit:"in",format:"a4",orientation:"portrait"}},E=je().set(Y).from(K);if(R==="blob")return E.outputPdf("blob");if(R==="save")return E.save()},Ue=()=>{var P;const d=(P=c())==null?void 0:P.company_info,{id:x}=ue(),[S,o]=$.useState(0);$.useRef(null),$.useState(null);const[A,Z]=$.useState("english"),[k,R]=$.useState(""),[a,J]=$.useState(0),{showToast:M}=ye(),m=fe(),[t,K]=$.useState({project_name:"",customer:{name:"",address:"",mobile:"",gst_number:""},date:"",items:[],discount:0,amountPaid:0,paymentMode:"",invoiceStatus:"",finalAmount:0,totalAmount:0,invoice_number:"",status:"",deliveryDate:"",invoiceType:"",cgst:0,sgst:0,gst:0,igst:0,invoice_rules:[],ref_id:"",po_number:""}),Y=()=>{m(`/edit-order/${x}`)},E=n=>{if(n===0)return"Zero";const l=["","One","Two","Three","Four","Five","Six","Seven","Eight","Nine"],j=["Ten","Eleven","Twelve","Thirteen","Fourteen","Fifteen","Sixteen","Seventeen","Eighteen","Nineteen"],v=["","","Twenty","Thirty","Forty","Fifty","Sixty","Seventy","Eighty","Ninety"],b=r=>{let i="";return r>=100&&(i+=l[Math.floor(r/100)]+" Hundred ",r%=100),r>=20?(i+=v[Math.floor(r/10)],r%10>0&&(i+=" "+l[r%10])):r>=10?i+=j[r-10]:r>0&&(i+=l[r]),i.trim()};let g="",s=Math.floor(n);if(s>=1e7){const r=Math.floor(s/1e7);g+=b(r)+" Crore ",s%=1e7}if(s>=1e5){const r=Math.floor(s/1e5);g+=b(r)+" Lakh ",s%=1e5}if(s>=1e3){const r=Math.floor(s/1e3);g+=b(r)+" Thousand ",s%=1e3}return s>0&&(g+=b(s)),g.trim()+" Rupees Only"},ne=()=>{window.print()},_=async()=>{var n,l,j,v,b,g;try{const s=await ve(`/api/order/${x}`);console.log("Fetched order:",s);const r=s.paymentType===0?"Cash":"Online (UPI/Bank Transfer)";let i="";switch(s.orderStatus){case 0:i="Cancelled Order";break;case 1:i="Delivered Order";break;case 2:i="Order Pending";break;case 3:i="Quotation";break;default:i="Unknown Status"}const u=s.discount||0,w=Number(s.finalAmount||0).toFixed(2),L=Number(s.totalAmount||0).toFixed(2),U=w-(s.paidAmount||0);o(Math.max(0,U));const C=Number(s.cgst||0),G=Number(s.sgst||0),F=Number(s.igst||0),I=Number(s.gst||0),N=Number(s.totalAmount||0),B=N>0?Math.round(C/N*100*100)/100:0,Q=N>0?Math.round(G/N*100*100)/100:0,H=N>0?Math.round(F/N*100*100)/100:0,X=N>0?Math.round(I/N*100*100)/100:0;K({project_name:((n=s.project)==null?void 0:n.project_name)||"N/A",customer:{name:((l=s.project)==null?void 0:l.customer_name)||"N/A",address:((j=s.project)==null?void 0:j.work_place)||"N/A",mobile:((v=s.project)==null?void 0:v.mobile_number)||"N/A"},gst_number:((b=s.project)==null?void 0:b.gst_number)||"N/A",pan_number:((g=s.project)==null?void 0:g.pan_number)||"N/A",date:s.invoiceDate||"",items:(s.items||[]).map(p=>({work_type:p.product_name||p.work_type||"N/A",qty:p.dQty||p.qty||0,uom:p.uom||"N/A",price:p.dPrice||p.price||0,total_price:p.total_price||0,remark:p.remark||"",gst_percent:Number(p.gst_percent)||0,cgst_amount:Number(p.cgst_amount)||0,sgst_amount:Number(p.sgst_amount)||0})),discount:u,amountPaid:s.paidAmount||0,paymentMode:r,invoiceStatus:i,totalAmount:L,finalAmount:w,cgst:C.toFixed(2),sgst:G.toFixed(2),gst:I.toFixed(2),igst:F.toFixed(2),cgstPercentage:B,sgstPercentage:Q,igstPercentage:H,gstPercentage:X,ref_id:s.ref_id,po_number:s.po_number||"",terms_and_conditions:s.terms_and_conditions||"",payment_terms:s.payment_terms||"",note:s.note||"",invoice_number:s.invoice_number||"N/A",status:s.orderStatus,deliveryDate:s.deliveryDate||"",invoiceType:s.invoiceType||3,invoice_rules:Array.isArray(s.invoice_rules)?s.invoice_rules:[]}),J(w),R(E(w))}catch(s){console.error("Error fetching order data:",s),M("danger","Error fetching invoice details")}};console.log("Customer GST Number:",t==null?void 0:t.gst_number),$.useEffect(()=>{_()},[x]);const O=async()=>{try{const n=await pe(t.finalAmount,t.invoice_number,t.customer.name,t,S,k,A,"blob"),l=URL.createObjectURL(n),j=encodeURIComponent(`*Invoice from ${(d==null?void 0:d.company_name)||"Company"}*
...
`).map((n,l)=>e.jsx("li",{className:"text-dark",children:n},l)):e.jsx("li",{className:"text-muted",children:"No terms and conditions provided."})})]})}),e.jsx("div",{className:"row section mt-3",children:e.jsx("div",{className:"col-md-12 text-center",children:e.jsx("p",{children:"This bill has been computer-generated and is authorized."})})}),e.jsxs("div",{className:"d-flex justify-content-center flex-wrap gap-2",children:[e.jsx(V,{color:"danger",variant:"outline",className:"d-print-none flex-fill",onClick:Y,children:"Edit Order"}),e.jsx(V,{color:"primary",variant:"outline",onClick:ne,className:"d-print-none flex-fill",style:{display:"none"},children:"Print"}),e.jsxs(Te,{className:"mb-2 d-print-none flex-fill",value:A,onChange:n=>Z(n.target.value),style:{maxWidth:"200px",display:"none"},children:[e.jsx("option",{value:"english",children:"English"}),e.jsx("option",{value:"marathi",children:"Marathi"}),e.jsx("option",{value:"tamil",children:"Tamil"}),e.jsx("option",{value:"bengali",children:"Bengali"})]}),e.jsx(V,{color:"success",variant:"outline",onClick:()=>D(A),className:"d-print-none flex-fill",children:"Download PDF"}),e.jsx(V,{color:"success",variant:"outline",onClick:()=>O(),className:"d-print-none flex-fill",children:"Share on WhatsApp"})]})]})})]})};export{Ue as default};
